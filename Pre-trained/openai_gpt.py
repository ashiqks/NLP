...
Original file is located at
    https://colab.research.google.com/drive/1Jvqe9m28t5u7nfD2yeJ20RlY0Pwt-o24
"""

'''

Classification of text sentiments using OpenAI's
pretrained classification model using pytorch
...
from pytorch_pretrained_bert import OpenAIGPTTokenizer, OpenAIGPTModel, OpenAIGPTConfig
import requests 
import pandas as pd
import regex as re
...
class OpenAIGPT_Classification(nn.Module):
  def __init__(self, num_labels=2):
    ...
    self.openai_gpt = OpenAIGPTModel.from_pretrained('openai-gpt').to(device)
...
x_train = x_train.values.tolist()[0:10]
x_test = x_test.values.tolist()[:2]

# One-hot encode the target variable using pandas get_dummies method
y_train = pd.get_dummies(y_train).values.tolist()[:10]
y_test = pd.get_dummies(y_test).values.tolist()[:2]
...
optim_model = optim.Adam([
    ...
    {'params': model.openai_gpt.parameters(), 'lr': lr_openai_gpt},
...
def training(model, criterion, optimizer, scheduler, num_epochs=100):
  ...
  for i in progress_bar(range(num_epochs)):
    ...
    for phase in ['train', 'test']:
      ...
      for inputs, sentiments in progress_bar(dataloader_dict[phase]):
